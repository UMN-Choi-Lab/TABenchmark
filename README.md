...
Twenty-one assignment models and five OD-estimation baselines ship today, spanning white-box
solvers, a stochastic track, elastic and combined-distribution demand, day-to-day dynamical
systems (route-swap, link-based, and cost-smoothing SUE), a boundedly-rational equilibrium, a capacity-constrained equilibrium, and the first learned model — every one certified
...
| Tests | 292 tests: analytic Braess UE/SO and two-route logit/probit/elastic oracles, a symmetric-bipartite combined distribution+assignment oracle (with the certificate's aggregate-multicommodity limitation pinned transparently), the day-to-day route-swap convergence + Beckmann-Lyapunov monotonicity, its logit-SUE sibling reaching the analytic logit fixed point with a monotone Fisk-Lyapunov objective, the link-based day-to-day invariance (link flows stay in the OD polytope every day) reaching the same certified UE as the route-swap dynamics, the Horowitz cost-smoothing day-to-day SUE converging to the analytic logit split below its stability threshold and settling into a period-2 limit cycle above it, the boundedly-rational two-route band edge (and its necessary-not-sufficient certificate); best-known-solution regressions on Sioux Falls, Anaheim, Barcelona, Winnipeg; cross-family solver agreement; conjugacy-identity and golden-hash regressions |
