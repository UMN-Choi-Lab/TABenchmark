...
graph TD
  ...
  n_zhoudtalite9651(["Zhou 2014"]):::c10
  ...
  n_leblancefficient294 --> n_zhoudtalite9651
  n_lopezmicroscopic2236 --> n_zhoudtalite9651
...
### Li, Wang & Nie (2024) — Wardrop Equilibrium Can Be Boundedly Rational: A New Behavioral Theory of Route Choice

`dtd-cumlog` · **shipped** · day-to-day cumulative-logit dynamics converging to deterministic Wardrop UE at finite r · `[li2024wardrop]`

The first boundedly-rational logit route-choice day-to-day process whose global limit is EXACT deterministic Wardrop UE at a FINITE exploitation parameter -- filling the empty cell in the (choice map x limit point) matrix where every other logit-choice day-to-day model rests at SUE.

**What it does differently.** Classical day-to-day models value a route by the cost AVERAGED over history (Horowitz 1984; the successive-average scheme, whose fixed point is SUE). CumLog changes one line: it ACCUMULATES the experienced cost. That single change makes a boundedly-rational logit choice map (travelers assign strictly positive probability to acceptable suboptimal routes every day) converge to exact WE at finite r -- with no equilibrium-concept relaxation and no indifference band. On the WE support the accumulated valuation DIFFERENCES converge to finite nonzero constants (explaining why equal-cost routes carry unequal probabilities -- resolving Harsanyi's instability), while non-WE routes' valuations diverge to +inf, so CumLog eliminates them even at finite r (an averaged-cost logit cannot, unless r->inf; its limit is SUE). Its bounded rationality is PROCESS-level (the adjustment path), disjoint from br-ue's CONCEPT-level indifference band (adr-038 vs adr-008).

**Formulation.** `p_k = exp(-r s_k)/sum_k' exp(-r s_k') (Eq.2); s_t = s_{t-1} + eta_t c(p_{t-1}) (Eq.6, accumulate) vs s_t = (1-eta_t)s_{t-1} + eta_t c(p_{t-1}) (Eq.4, average -> SUE); s stored min-normalized per OD (Variant 1); WE limit under eta_t->0,sum eta_t=inf (any r) OR eta_t=eta<1/(2rL).`

**Validation.** SHIPPED as `dtd-cumlog` (paradigm day_to_day): the STATE is a per-OD route-VALUATION vector s over gp/dtd-swap column-generated working sets; the day map is the logit p=softmax(-r s), the emitted flow is the full-demand logit load f_k=d_w p_k (demand-feasible every day, node-balance~0), and the update ACCUMULATES the experienced route cost s<-s+eta_t c(p), stored min-normalized per OD (paper Variant 1/Remark 2 -- mathematically identical, keeps used-route floats finite while dropped-route valuations diverge). Certified by the STANDARD UE relative gap (fixed point = Wardrop UE, the paper's own Eq.11 measure); NO new certificate and NO new scenario field, so the golden Braess content hash is byte-identical; the model self-reports the SAME (TSTT-SPTT)/TSTT the harness recomputes so the P1 honesty diff passes to float precision; SUE/elastic/combined/BR scenarios refused (the limit is WE regardless of r, and scenario.sue_theta is NEVER mapped to r). HEADLINE (the accumulation-vs-averaging distinctness gate, Remark 3): on ONE two-route instance with identical machinery and the same r=1, the `accumulate` flag flips the limit -- accumulation drives the UE gap<1e-6 with f_A->2.5 (exact WE), while averaging rests at the analytic binary-logit SUE f_A->2.373888 (brentq root at dispersion r, MATCHED TO SIX DIGITS) with UE gap>0.01. `accumulate=False` is a comparison/regression knob, NEVER a shipped SUE mode (dtd-horowitz remains the logit-SUE row). Further validated: exact Braess UE [4,2,2,2,4] (gap<1e-6) and the paper's three-parallel-link example (2,1,0); harmonic r-independence (r in {1,10,40} all reach the Braess UE, Theorem 1(i)); the constant schedule converges (small step) or diverges (large step, preserved not damped, mirroring dtd-horowitz), with the reported eta_heuristic_scale=1/(2r max_a t'_a) shown to be a step-scale REFERENCE, not the Theorem 1(ii) bound in either direction -- it is flow-independent (=0.05 on Braess at r=1), yet demand-6 eta0=0.08 ABOVE it converges to machine precision while demand-60 eta0=0.02 BELOW it diverges (true L is the demand-scaled route-cost Lipschitz constant, uncomputed here); s0-independence (different finite s0 reach the SAME unique WE link flow on Braess through DIFFERENT strategy entropies on Sioux Falls via init_valuation_scale -- this row's extension of the paper's 3N4L Fig.10 entropy histogram, whose Sioux Falls half of Sec.6.4 reports Fig.11 used-route counts); the valuation-divergence signature (at the shipped r=0.25/constant-eta=1/400-day config: used-route spread stabilizes finite ~52.6, dropped-route valuation max diverges ~85723 on Sioux Falls). DISCLOSED CONSEQUENCE: the paper enumerates the full route set up front, but under this benchmark's column generation a never-shortest non-WE route is never generated -- the model reaches correct WE by excluding it, but the dropped-route-divergence half of the signature is observable only where transient shortest-path switching sheds a GENERATED route (Sioux Falls, active-route count descending to the WE support, Fig.11), not on tiny fixed networks (the same working-set-vs-full-set distinction dtd-swap-sue documents). Sioux Falls converges slowly (CumLog uses no higher-order info -- the primary acknowledges this), so the scaling test uses a constant eta=1 step (the paper's Sec.4.3/6.1 setting) at r=0.25 -- smaller than the paper's r=2.5 Sioux Falls runs (Sec.6.4), de-saturating faster under column generation -- and a loose Beckmann tolerance. Primary Li, Wang & Nie 2024 (TS 58(5):973-994, DOI 10.1287/trsc.2023.0132) READ IN FULL -- the STRONGEST sourcing in the dtd family, no attributed-unread caveat; open anchors arXiv:2304.02500 v2 (Theorem/Assumptions content identical to the journal, numbering-only drift) + NSF PAGES accepted manuscript.

*Builds on:* Horowitz 1984, Smith 1984, Fisk 1980, Cantarella & Cascetta 1995.
...
`het-gnn` · **shipped** · learned UE surrogate with heterogeneous message passing · `[liu2024end]`
...
**Validation.** Phase 3 (torch, optional extra). Shipped as a LEAN VARIANT (flagged like implicit-ue-nn / dtd-stochastic; the TR-C primary is paywalled/unread, formulation cross-verified with zero discrepancies from arXiv:2310.13193v3 + the NSF PAR accepted manuscript, both read in full -- adr-026): hand-rolled heterogeneous message passing (V/R-Encoder segment-softmax attention over real + virtual OD edges, plain-torch scatter, no torch-geometric) with size-agnostic node-kernel features [production, attraction, in/out-degree] replacing the paper's non-equivariant |V|+2 OD-row featurization, an edge-MLP flow/capacity-ratio head, and the paper's composite loss (w_a=1, w_f=0.005, w_c=0.05) trained by plain backprop on the synthetic-net family + Eq-15 demand-rescaling augmentations against bfw labels. Two harness-certified checkpoints: (i) the paper-faithful RAW emission (sp_calls=0) is CENSORED feasible=0 -- its conservation loss L_c IS the L1 form of the harness node_balance_residual, 3-5 orders above tolerance (act three of ADR-006: soft conservation is not a constraint); (ii) a route-based feasible DECODE (a flagged REPO EXTENSION, not in the paper) projecting the raw flows onto the per-OD scaled-simplex product over implicit-ue's column-generated routes, earning a real certified gap. Anchors: A2 raw-censored + decoded-feasible, A4 permutation equivariance 1e-8 (the paper's featurization fails at 21.5), decode-as-projection + no-limit-cycle, in-family loss reduction + conservation ablation. Honest held-out: implicit-ue-nn's learned-cost fixed point certifies a better gap than het-gnn's regression+projection at matched route sets, and a converged solver beats both (the identifiability caveat, scoped in-family).
...
**Validation.** Integrated download-on-demand as the cross-domain axis (ADR-033): 17 of 20 real US-city instances shipped AS-PUBLISHED via HTTP byte-range extraction of only the per-city AequilibraE trio (network.csv + od_demand.aem + assignment_result.csv) from the 276 MB CC-BY figshare zip -- never vendored, per-file SHA-256, on a registry deliberately separate from the CI-prefetched TNTP one so no CI run pulls the whole archive; Honolulu + San Francisco are CI-sized rungs, the other 15 local-only. The dataset's wrong-centroid defect (the published AequilibraE runs injected demand at node ids 1..Z, not the tract centroids -- the authors' own AequilibraE_assignment.py: g.prepare_graph(np.arange(zones)+1)) is a documented known-defect that also excludes 3 unbuildable cities (Washington, Pittsburgh, Phoenix -- named, their centroids partly absent as low-id nodes). The published AequilibraE flows are a LOOSE reference (own relative gap ~1e-3, ~11 orders looser than a TNTP best-known), never a best-known oracle, and cross-solver TransCAD agreement is NOT claimed; the honest validation is builder invariants on all 20 cities, published-flow conservation recomputed <=3e-10 (worst New York ~2e-10, summation-order sensitive), and the repo's own bfw reaching a far tighter gap on the identical instance -- cross-implementation agreement (Honolulu rgap 1.07e-4, correlation 0.99992). numpy/stdlib-only (no pandas); the data node stays shipped:false / not-a-solver, matching the TNTP (stabler2016transportation) precedent.
...
### Zhou & Taylor (2014) — DTALite: A queue-based mesoscopic traffic simulator for fast model evaluation and calibration

`dtalite-tap` · **shipped** · Static deterministic user equilibrium via a link-based Frank-Wolfe loop (Bar-Gera FW.zip-derived TAPLite) on the repo BPR, exactly represented · `[zhou2014dtalite]`

The second external engine and the first IDENTITY-map row: the PyPI DTALite wheel's static Frank-Wolfe assignment() wrapped as a benchmark model, its per-link BPR VDF mapping the repo cost EXACTLY so Sioux Falls power-4 becomes the marquee anchor.

**What it does differently.** Where sumo-marouter's hardcoded linear class law forced a mapping floor, DTALite's per-link vdf_fftt/vdf_alpha/vdf_beta IS the repo BPR t=fft*(1+b*(v/cap)^power), so the compile map is the IDENTITY on (fft,b,power,capacity) and BPR power-4 encodes with no cost-model approximation -- the power!=1 refusal that blocked marouter has no analog. The catch is honest and measured: the engine's Armijo line search collapses to step 0 within a few iterations, so the certified gap freezes at a floor (Braess ~1.2e-2, Sioux Falls ~5.0e-3) far above a converged white-box solver's ~1e-16 -- the row is the wheel-engine-as-shipped, and the ceiling is its line-search stall, NOT the mapping.

**Formulation.** `engine VDF t=vdf_fftt*(1+vdf_alpha*(V/(lanes*cap*period*plf))^vdf_beta); lanes=1, period=1h, plf=1, vdf_fftt=fft, vdf_alpha=b, vdf_beta=power, capacity=cap -> t=fft*(1+b*(v/cap)^power) exactly; flows from link_performance.csv volume; harness recomputes RG/AEC/Beckmann under true BPR (P1).`

**Validation.** SHIPPED as `dtalite-tap` (paradigm heuristic, the aon/sumo-marouter precedent; registered when the optional `DTALite` wheel is present, guarded-imported so the numpy/scipy core stays dependency-free; a FIFTH CI job pins DTALite==0.8.1 since the FW/VDF behavior is a bundled .so that can shift under the >=0.8 floor). The second external engine (adr-029), verified by running the PyPI wheel 0.8.1 in-tree: DTALite.assignment() -> DTA_AssignmentAPI() is a static link-based Frank-Wolfe TAP loop the wheel's own TAPLite.cpp derives from Bar-Gera's FW.zip -- NOT the 2014 paper's mesoscopic queue DUE (that is the separate simulation() entry, a named non-goal). The compile map is the IDENTITY: the engine's per-link VDF reduces to the repo BPR under lanes=1 (the measured lanes^2 trap: the engine divides the ratio by lanes^2*capacity), a 1-hour demand period, vdf_plf=1, and lowercase columns carrying the repo (fft,b,power,capacity) verbatim (the engine is case-sensitive -- an uppercased column is silently ignored). Measured anchors, all certified by the P1 harness under the true BPR: A1 Braess (the fft=1e-6/alpha=1e7 sentinels survive the parse -- fft=1e-6 echoes as 0.0 by 4-decimal display but the internal cost still matches) feasible=1, node_balance 0, certified relative_gap 1.18e-2 at 100 iters FROZEN from ~iter 40 (the line-search stall, the honest ceiling), golden Braess hash cf00f411... byte-identical; A2 the MANDATORY cost-matched anchor -- the engine's own travel_time equals the repo BPR at the emitted flows on EVERY link (the identity map's payoff, no mapping floor to separate) to max relative 7.1e-6 on Braess (the sentinel links; clean links ~1e-15) and 2.2e-5 on Sioux Falls power-4 (the engine's 4-decimal travel_time column), so the certified gap is pure FW truncation; A3 two-route deterministic UE feasible=1 relative_gap 0.0 (EXACT UE f_A=2.5, NO theta calibration needed, contrast marouter); A4 the Sioux Falls power-4 MARQUEE (marouter-impossible) feasible=1 all 76 links matched relative_gap 5.03e-3 frozen from ~iter 100 flow-NRMSE 1.58% vs best-known -- the first external engine on the power-4 ladder. Negative controls: iterations=1 runs the pure all-or-nothing load (0 FW line searches; measured number_of_iterations=N runs N-1 iterations, output labeled iteration_no=N, N=0 is accessibility mode/empty so the floor is 1) certifying feasible=1 with a worse gap (Braess RG 1.9e-1) and pinning the dial live RG(1)>RG(100); byte-determinism (link_performance.csv byte-identical across reruns, deterministic=True seedable=False -- the engine exposes no seed to pin); a converged bfw beats the line-search floor by orders of magnitude on the convergence axis. Read-back (the house compile-check, free here): link_performance echoes vdf_fftt/vdf_alpha/vdf_beta/vdf_plf/link_capacity per link, verified equal to declared within the engine's float32/4-decimal precision with every repo link matched exactly once by (from,to) and the row count equal to the link count -- so returncode==0 is never trusted (the engine exits 0 on missing files, dropped links, zone!=node with zero/garbage flows). Subprocess wrapper is MANDATORY (three engine hazards, measured): ExitMessage does getchar()+exit() in-process (a bad input would HANG on open stdin or KILL the host -- the child runs stdin=DEVNULL), a second assignment() call in one process doubles the flows (global state), and it reads/writes CWD; so [sys.executable,-c,'import DTALite; DTALite.assignment()'] runs cwd=fresh tempdir, OMP_NUM_THREADS=1 (the default all-cores OpenMP is pure spin: Sioux Falls 8.5s -> 0.1s), one wall deadline across write/run/parse (a tiny budget RAISES RuntimeError, never a hang or a laundered feasible=0). Refusals name the field: sue_theta/elastic/combined/br/side-cap/link-interaction/multiclass (the natural sprint-2 extension via mode_type.csv), nonzero fixed_cost (the toll hook exists but its vot conversion is unvalidated and a negative toll kills the host -- refuse now, record the lift), capacity<=1e-4 (the engine's silent-drop threshold), an sp_calls-only budget (no Dijkstra count exposed; sp_calls disclosed as 0). The engine's self-reported gap is (TSTT-SPTT)/SPTT (SPTT denominator) not the repo (TSTT-SPTT)/TSTT -- differ by 1/(1-RG), recorded as self_report provenance, never gated on. Hardened by a three-lens adversarial review (1 CRITICAL + 3 MAJOR + MINORs, all fixed + regression-pinned): CRITICAL -- an ungrouped link.csv corrupts the engine's FirstLinkFrom adjacency (permuted Braess certifies WRONG flows RG 0.208; permuted SiouxFalls = infinite loop hanging solve() under an iterations-only budget) -> the adapter now writes links SORTED by (from,to), permutation-invariant; MAJORs -- the engine clamps capacity at fmax(0.1,cap) in the cost law only (a cap in (1e-4,0.1) equilibrates under a different BPR, A2 rel 0.93) -> refuse cap<0.1 naming the clamp; the ~1e7 BIG-M ceiling silently ZEROES an over-ceiling OD -> a per-origin MASS GATE raises; a corrupt-but-parseable output raised a raw ValueError escaping solve() -> every cell + the file read wrapped to the contract RuntimeError with stderr tail; MINORs -- a runtime A2 cost-match (engine travel_time==repo BPR on every link, rel 1e-3) is now the LIVE gate closing the echo check's sub-atol blind spot AND catching the clamp; the wall deadline covers the parse phase; the summary parser never leaves a torn (iter,gap) pair; the self-gap 1/(1-RG) claim scoped to the converged regime (verified 6 digits on siouxfalls/winnipeg/barcelona; it can be negative off it). First big-net exposure: Winnipeg (2836 links, pow 0-6.87) feasible=1 RG 1.176e-3 A2 5.2e-4; Barcelona (2522 links, pow to 16.83) feasible=1 RG 5.48e-4 A2 7.6e-4. 26 tests (importorskip DTALite, EXACT case) including the banner-suppression + core-install guard (import tabench prints NOTHING and the model unregisters when DTALite is blocked via a meta_path finder). The wheel is Apache 2.0 in PyPI metadata but the source repo LICENSE file is empty (0 bytes, SPDX NOASSERTION) -- recorded; no engine code vendored, no TNTP sample data committed. zhou2014dtalite anchors the software LINEAGE (tool-paper discipline, the lopez2018 precedent): the 2014 Cogent paper describes a mesoscopic simulator with NO BPR and no equilibrium benchmark numerics; assignment() executes classic static FW on BPR, so the row validates the ADAPTER + engine artifact, NEVER the paper's numerics.

*Builds on:* LeBlanc, Morlok & Pierskalla 1975, Lopez, Behrisch, Bieker-Walz et al. 2018.
