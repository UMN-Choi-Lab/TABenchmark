...
def test_prune_tol_changes_retained_flows(siouxfalls):
    ...
    columns are dropped from the working set, and dropping different columns changes
    the retained set and thus the (non-converged) link-flow trajectory. On Sioux Falls
    a loosened prune_tol=1e-6 yields link flows NOT byte-identical to the tight default
    1e-14 (measured max|diff| ~4), while staying feasible. MUTANT KILL: hardcoding
    prune_tol (ignoring the factor) collapses the two runs to byte-identical."""
    def flows(prune_tol):
        trace = Trace()
        LinkBasedDTDModel(prune_tol=prune_tol).solve(
            siouxfalls, Budget(iterations=60), RngBundle(0), trace
        )
...
def test_prune_tol_default_is_pinned(siouxfalls):
    ...
    def flows(model):
        trace = Trace()
        model.solve(siouxfalls, Budget(iterations=60), RngBundle(0), trace)
        return trace.final.link_flows

    np.testing.assert_array_equal(
        flows(LinkBasedDTDModel()), flows(LinkBasedDTDModel(prune_tol=1e-14))
    )
